...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "Usage: $0 CONFIGS/H51_SMOKE.LOCAL.YAML [BEEID_DINO_PYTHON]" >&2
  exit 2
fi
...
"${PYTHON_BIN}" -m beeid.cli h51-synthetic-smoke
"${PYTHON_BIN}" -m beeid.cli h51-run-all --config "$1" --models resnet50 dinov3
