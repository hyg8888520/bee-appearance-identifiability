...
if [[ $# -lt 1 || $# -gt 2 ]]; then
  echo "Usage: $0 CONFIGS/H5.LOCAL.YAML [BEEID_DINO_PYTHON]" >&2
  exit 2
fi
...
"${PYTHON_BIN}" -m beeid.cli h5-validate-protocol --protocol configs/h5_protocol.lock.yaml --checksum configs/h5_protocol.lock.sha256
"${PYTHON_BIN}" -m beeid.cli h5-run-all --config "$1" --models resnet50 dinov3 --confirm-full
